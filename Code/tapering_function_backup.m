function [tapering_coefficient,output] = tapering_function(corr_field,corr_field_shuffled,kalmanOptioins,varargin)
...
%
if isempty(tf)    
    [tapering_coefficient,output] = three_pieces_thresholding(corr_field,corr_field_shuffled,kalmanOptioins,freepara);
else
    eval(['fh = ' tf]);
    tapering_coefficient = fh(corr_field,corr_field_shuffled,kalmanOptioins,freepara);
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% embedded function
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [tc, output] = three_pieces_thresholding(cf,cf_shuffled,ko,fp,varargin)
        % cf = corr_field, nl = noise_level, tc = tapering_coefficient
        % ko = kalmanOptioins, fp = freepara;
        
        tc = zeros(size(cf));
        ensize = ko.ensembleSize;
        ...
        for i = 1 : size(cf,2) % size(cf,2) corresponds to observation size
            ...
            if strcmpi(ko.thresholding_mode,'hard')
                cutoff_point_lower = cutoff_point_upper; % hard thresholding
            else %
                % soft thresholding. "ko.nstd" stands for number of STDs (e.g., ko.nstd = 1)
                cutoff_point_lower = ko.nstd * est_noise_std; 
            end
